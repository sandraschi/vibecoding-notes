...
- [rulebooks-and-agent-instruction-files.md](rulebooks-and-agent-instruction-files.md) — the wrapper format (`.cursorrules` → `CLAUDE.md` → `AGENTS.md` → skills) keeps churning, but the content categories (platform gotchas, output hygiene, safety gates, workflow sequencing) persist. Prune stale rules as diligently as you add new ones — a rule fixing a limitation the model no longer has is clutter at best, wrong guidance at worst.
