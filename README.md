...
- [ai-dev-is-universal-acid.md](ai-dev-is-universal-acid.md) — the specialist-in-a-box argument, generalized past devops: AI dissolves the learning-curve wall for any tool whose difficulty was mostly a huge memorized surface (Blender's thousand-page docs, iOS/Swift's six-month onboarding). The remaining "taste" wall splits into recognition (mostly dissolved) vs. origination (still the real gap) — see the blooper-detector note below.
- [ai-as-ui-blooper-detector.md](ai-as-ui-blooper-detector.md) — the fakefind pattern aimed at aesthetics: a checklist (tiny fonts, buried help, no onboarding, weak hero, dead-end empty states, non-obvious controls) turns "does this look good" from a vague vibe check into concrete, reliable findings.
...
- [rulebooks-and-agent-instruction-files.md](rulebooks-and-agent-instruction-files.md) — the wrapper format (`.cursorrules` → `CLAUDE.md` → `AGENTS.md` → skills) keeps churning, but the content categories (platform gotchas, output hygiene, safety gates, workflow sequencing) persist. Prune stale rules as diligently as you add new ones — a real example: "always use FastMCP 2.1" sitting unnoticed at rule #25 for months. Weed weekly, not someday.
- [ai-of-the-gaps-and-the-hype-reflex.md](ai-of-the-gaps-and-the-hype-reflex.md) — don't cite a current AI limitation as permanent ("LLMs can't write good PRs" was true, then wasn't, within a year); and tame AI's default logorrhea/hype reflex with one rule in your AGENTS.md instead of linting it out after the fact.
